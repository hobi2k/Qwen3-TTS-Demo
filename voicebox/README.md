...
이 폴더는 예전 명령어를 깨지 않기 위한 `VoiceBox` 호환 래퍼 디렉터리입니다.

핵심 원칙:

- canonical 구현은 `Qwen3-TTS/finetuning`, `Qwen3-TTS/fusion`, `Qwen3-TTS/inference`에 둡니다.
- 이 폴더의 파일들은 canonical script로 실행을 넘깁니다.
- 공통 런타임 로직은 `Qwen3-TTS/inference/voicebox/runtime.py`에 둡니다.
- 새 VoiceBox 기능 수정은 이 폴더가 아니라 `Qwen3-TTS` 안의 역할별 canonical script에 먼저 반영합니다.
...
- `sft_plain_custom_voice_12hz.py` (호환 래퍼)
...
```bash
.venv/bin/python Qwen3-TTS/inference/voicebox/clone.py \
