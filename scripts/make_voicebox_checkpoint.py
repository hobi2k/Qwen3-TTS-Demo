#!/usr/bin/env python3
"""Embed a speaker encoder into a CustomVoice checkpoint to create VoiceBox."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--input-checkpoint", required=True, help="CustomVoice checkpoint directory to upgrade.")
    parser.add_argument("--speaker-encoder-source", required=True, help="Base 1.7B checkpoint that contains speaker_encoder.* weights.")
    parser.add_argument("--output-checkpoint", required=True, help="Destination checkpoint directory.")
    return parser.parse_args()


def load_config(path: Path) -> dict:
    """Load one checkpoint config file.
...
def main() -> None:
    """Copy a CustomVoice checkpoint and embed speaker_encoder weights into it."""
    ...
    speaker_encoder_state = {
        key: value
        for key, value in speaker_source_state.items()
