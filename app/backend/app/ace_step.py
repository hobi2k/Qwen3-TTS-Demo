...
class AceStepComposer:
    ...
    def generate(
        ...
        *,
        ...
        """Backwards-compatible text2music entrypoint used by the legacy form.

        Older fields (``scheduler_type``, ``cfg_type``, ``omega_scale``,
        ``guidance_interval`` …) come from the original ACE-Step v1 API. They
        are forwarded as ``extra.legacy_params`` so the metadata stays useful
        for the gallery, but the v1.5 subprocess uses the modern names where
        possible (``inference_steps``, ``guidance_scale``, ``shift``…).
        """
        ...
        payload: Dict[str, Any] = {
            ...
            "duration": audio_duration if audio_duration and audio_duration > 0 else -1.0,
        ...
        return self.run(task="text2music", output_path=output_path, payload=payload)
