...
CustomVoice and VoiceBox stages. It lives beside upstream fine-tuning scripts
so the training code is easy to compare with ``sft_12hz.py`` and
``sft_custom_voice_12hz.py`` while still keeping the VoiceBox-specific logic in
one clearly named helper module.
...
UPSTREAM_FINETUNING_DIR = Path(__file__).resolve().parent
VOICEBOX_FAMILY = "voicebox"
CHECKPOINT_EPOCH_RE = re.compile(r"checkpoint-epoch-(\d+)$")

# Import pristine upstream modules without modifying the upstream tree.
