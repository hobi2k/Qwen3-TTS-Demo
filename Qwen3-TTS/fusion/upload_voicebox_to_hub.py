#!/usr/bin/env python3
"""Upload a VoiceBox checkpoint to Hugging Face Hub."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--repo-id", required=True, help="Target Hub repo id.")
...
def main() -> None:
    ...
    if not token:
        raise SystemExit("No Hugging Face token was found. Set HF_TOKEN or run `huggingface-cli login` before uploading.")

    api = HfApi(token=token)
    create_repo(repo_id=args.repo_id, repo_type="model", private=args.private, exist_ok=True, token=token)
    api.upload_folder(
