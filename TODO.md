...
## 4-1. ACE-Step 음악 생성 운영 검증

ACE-Step 작곡 기능은 실제 로컬 ACE-Step checkout과 전용 Python 환경을 사용합니다.

완료된 작업:

- `./scripts/download_models.sh ace-step` 프로필 추가
- `vendor/ACE-Step`, `.venv-ace-step`, `data/models/ace-step` 경로 표준화
- `/api/music/ace-step/generate` 백엔드 엔드포인트 추가
- `scripts/run_ace_step_generate.py` 별도 프로세스 runner 추가
- `ACE-Step 작곡` 프런트 탭 추가
- 생성 결과를 `생성 갤러리`에 저장
- 문서 [22-ace-step-music.md](./docs/cookbook/22-ace-step-music.md) 추가

남은 운영 과제:

- 실제 ACE-Step checkpoint를 내려받은 환경에서 긴 곡 생성 smoke test
- 개인 Hugging Face mirror에 `ace-step/` prefix로 checkpoint/cache 업로드
