...
- `QWEN_EXTENSIONS`가 있으면 해당 폴더를 우선 사용
- 없으면 저장소 루트의 `qwen_extensions` 사용
- 같은 상대 경로가 없을 때만 `vendor/Qwen3-TTS` 내부 legacy mirror 사용

이렇게 나누면 `vendor/Qwen3-TTS` 업스트림 코드는 비교 기준으로 남기고, 제품 전용 확장 코드는 한곳에서 유지보수할 수 있습니다.
...
- `contentvec` embedder와 `rmvpe.pt` predictor는 `scripts/download_models.sh`가 `vendor/Applio/rvc/models/` 아래에 미리 준비합니다.
- subprocess의 `MPLCONFIGDIR`은 프로젝트 내부 `data/cache/matplotlib`로 고정합니다.
...
VoiceStudio는 Qwen, S2-Pro, ACE-Step, MMAudio, VibeVoice, Applio/RVC를 같은 백엔드에서 오갑니다.
이때 GPU memory를 오래 잡는 runtime을 명확하게 내리지 않으면 다음 기능으로 넘어갈 때 CUDA allocator 오류나 OOM이 발생할 수 있습니다.

- `GET /api/runtime/status`
  현재 FastAPI 프로세스의 Qwen/Qwen-ASR cache, 백엔드가 자동 시작한 S2-Pro server process, CUDA memory 상태를 반환합니다.
- `POST /api/runtime/unload?include_s2_pro=true`
  Qwen/Qwen-ASR cache를 비우고 Python CUDA cache를 반환합니다. `include_s2_pro=true`이면 백엔드가 시작한 Local S2-Pro Fish Speech 서버도 종료합니다.

자동 unload 정책:

- Qwen 계열 요청은 필요한 모델을 FastAPI 프로세스 안에서 lazy-load하고 재사용합니다.
- S2-Pro 요청은 Qwen cache만 먼저 비웁니다. 같은 S2-Pro 요청 안에서 S2 서버를 스스로 꺼 버리면 매번 재시작되기 때문입니다.
- ACE-Step, MMAudio, VibeVoice, Applio/RVC, Stem Separator 같은 비-S2 heavy engine 요청은 Qwen cache와 백엔드 관리 S2-Pro process를 모두 내린 뒤 실행합니다.
- 사용자가 별도로 띄운 외부 S2-Pro server는 백엔드가 종료하지 않습니다. `FISH_SPEECH_SERVER_URL`이 외부 서버를 가리키는 경우 운영자가 직접 관리해야 합니다.

### `/api/vibevoice/*`

VibeVoice는 저장소에 포함된 `vendor/VibeVoice` source와 전용 `.venv-vibevoice`로 실행하는 vendor wrapper입니다. 코드는 다운로드 시 git clone하지 않고, 모델 weight만 `data/models/vibevoice` 아래에서 찾습니다.
...
- `vendor/Applio`, `vendor/MMAudio`, `vendor/VibeVoice`는 서브모듈이 아니라 일반 tracked source입니다.
