...
가장 큰 변화는 당시 문서 기준 `Inference Lab`이라고 불리던 통합 추론 경로의 도입입니다. 현재 UI 라벨은 달라졌지만, 여기서 설명하는 “통합 모델 선택 추론” 개념은 그대로 유지됩니다.
...
이 통합 추론 화면은 stock 모델과 local fine-tuned 체크포인트를 **한 화면에서 모델처럼 선택**할 수 있게 합니다.
...
## 3. 통합 추론 화면에서 고급 파라미터까지 조정할 수 있게 되었습니다

현재 이 통합 추론 화면은 단순 데모 버튼이 아닙니다.  
상세 추론 제어판에 가깝습니다.
...
- 현재 생성 페이지의 모델 선택 영역에서 바로 선택 가능
...
당시 통합 추론 화면 안의 별도 카드로 노출되었습니다. 현재는 해당 기능이 스토리/하이브리드 생성 흐름으로 이어집니다.
...
이 요구가 당시 `Inference Lab`과 `/api/generate/model` 추가의 근본 이유였습니다.
