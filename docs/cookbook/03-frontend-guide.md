...
- `S2-Pro 태그 생성`
  인라인 bracket tag를 검색하고 대사에 삽입합니다.
- `S2-Pro 목소리 복제`
...
S2-Pro는 `Base`, `CustomVoice`, `VoiceBox`를 고르는 Qwen 모델 선택 화면과 목적이 다릅니다. 태그 기반 문장 제어와 로컬 Fish Speech `/v1/tts` 서버를 기준으로 별도 작업실을 둡니다.

원칙:

- Hosted API 키를 받지 않습니다.
