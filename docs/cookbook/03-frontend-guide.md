...
- `목소리 복제`, `목소리 설계`, `프리셋 기반 생성`, `스토리 스튜디오`는 서로 다른 작업이므로 탭도 분리합니다.
...
- 장문 대본 전용 화면
- 긴 텍스트를 한 번에 자연스럽게 읽는 용도

현재 원칙:

- `장면 1`, `장면 2` 같은 메타 문구를 사용자에게 설명하지 않습니다.
- 결과는 본문 아래에 붙여서 확인하는 편이 낫습니다. 오른쪽 빈 패널은 가능한 한 비워 둡니다.
- 장문 생성과 멀티 화자 데이터 생성은 같은 기능이 아닙니다.

즉 `스토리 스튜디오`는 장시간 대본용이고, 멀티 화자용 데이터 생성은 별도 기능으로 보는 것이 맞습니다.
