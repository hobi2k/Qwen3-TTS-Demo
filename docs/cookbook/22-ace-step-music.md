...
데이터셋 탭에서는 생성 갤러리 음성 또는 폴더 경로를 받아 `dataset.json`과 `audio/` 폴더를 먼저 만들 수 있습니다.
이미 전처리된 tensor 폴더가 있다면 `Prepared tensor` 미니 탭으로 바로 학습 폼에 연결합니다.
공통 데이터셋 생성 규칙은 [27-cross-engine-datasets.md](27-cross-engine-datasets.md)에 따릅니다.
