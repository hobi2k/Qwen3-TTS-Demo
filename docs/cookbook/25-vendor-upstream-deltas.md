...
- 현재 이 프로젝트는 Fish Speech upstream 파일 일부를 직접 수정합니다.

### Delta

Fish Speech는 wrapper script와 backend bridge로 통합하되, 로컬 S2-Pro 추론 성능과 시작 시간을 위해 아래 upstream 파일도 직접 patch합니다.

### 수정 파일

| 파일 | 변경 내용 | 이유 |
| --- | --- | --- |
| `vendor/fish-speech/fish_speech/models/text2semantic/inference.py` | `decode_n_tokens()`의 토큰 생성 루프에서 `with sdpa_kernel(SDPBackend.MATH):` wrapper 제거. 이에 따라 더 이상 쓰지 않는 `SDPBackend`, `sdpa_kernel` import도 제거. | 기존 vendored 코드가 매 토큰마다 PyTorch SDP backend를 `MATH`로 강제해서 flash/mem-efficient attention 경로를 막았습니다. RTX 5080 + torch 2.11/cu130 환경에서 실제 로그가 `0.11 tokens/sec`, `Bandwidth achieved: 0.50 GB/s` 수준으로 떨어졌으므로, PyTorch가 가능한 빠른 SDP backend를 선택하게 둡니다. |
| `vendor/fish-speech/tools/server/model_manager.py` | server warm-up의 `ServeTTSRequest.max_new_tokens`를 고정 `1024`에서 `int(os.getenv("FISH_SPEECH_WARMUP_TOKENS", "32"))`로 변경. | 로컬 서버 시작 시 `Hello world.` warm-up이 최대 1024 token 생성을 시도해 S2-Pro startup이 몇 분 이상 길어질 수 있었습니다. warm-up은 모델/codec 초기화 확인 용도이므로 기본 32 token으로 줄이고, 디버깅 시 환경변수로 늘릴 수 있게 합니다. |
...
- S2-Pro generation은 token loop가 병목이므로, attention backend를 `MATH`로 강제하면 16GB/24GB 여부와 별개로 프로덕션 사용이 어려운 지연이 생깁니다.
- server warm-up은 readiness 확인이지 품질 검증이 아니므로 긴 token budget을 쓰지 않습니다.

### 유지해야 하는 동작

- `vendor/fish-speech/fish_speech/models/text2semantic/llama.py` 안의 명시적 `sdpa_kernel(SDPBackend.FLASH_ATTENTION)` 경로는 유지합니다.
- `decode_n_tokens()`는 특정 SDP backend를 강제하지 않고 PyTorch 기본 선택에 맡깁니다.
- `FISH_SPEECH_WARMUP_TOKENS` 기본값은 `32`입니다. 서버 시작 벤치마크나 compile 확인이 필요할 때만 값을 늘립니다.
- Local S2-Pro provider는 여전히 `--half`를 기본으로 사용하며, `FISH_SPEECH_COMPILE=1`이면 upstream `--compile` 옵션을 전달합니다.
...
- upstream `inference.py`가 더 이상 `SDPBackend.MATH`를 강제하지 않거나 공식 option으로 빠른 backend를 제공하면 local patch를 제거할 수 있습니다.
- upstream `ModelManager.warm_up()`이 짧은 configurable warm-up을 제공하면 local warm-up patch를 제거할 수 있습니다.
...
./.venv/bin/python -m py_compile app/backend/app/fish_speech.py app/backend/app/main.py
nvidia-smi
```

라이브 S2-Pro 성능 검증은 사용자가 서버 실행을 허용한 상태에서만 진행합니다. 기존 회귀 기준은 `data/runtime/fish-speech-s2-pro.log`의 `Generated ... tokens ... tokens/sec`와 `Bandwidth achieved`를 비교합니다.
