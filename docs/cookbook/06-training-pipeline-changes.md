...
즉 지금은 "데이터셋만 만든다" 수준이 아니라, **어떤 종류의 모델을 학습할지 WEB UI에서 고르고, 실행 기록을 남기고, 결과 체크포인트를 다시 추론에 연결하는 구조**까지 포함합니다.
...
즉 데모의 dataset builder는 단순한 내부 포맷이 아니라, **업스트림 `prepare_data.py`가 바로 읽을 수 있는 학습 입력을 만들어 주는 도구**입니다.
...
즉 사용자는 "데이터셋 생성"과 "prepare codes"를 분리해서 이해해야 합니다.  
이 둘은 같은 일이 아닙니다.
...
즉 현재 `Training Lab`은 단순 입력 폼이 아니라, **업스트림의 서로 다른 학습 진입점을 선택하는 런처 역할**을 합니다.
...
## 7. WEB UI `Training Lab`이 실제 학습 제어판으로 확장되었습니다

현재 `Training Lab`에서 사용자는 아래 항목을 직접 선택할 수 있습니다.
...
4. 생성된 체크포인트를 `Inference Lab`에서 바로 선택
5. 실제 음성을 듣고 품질 비교
