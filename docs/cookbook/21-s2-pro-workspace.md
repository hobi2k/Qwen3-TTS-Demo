...
- `FISH_SPEECH_WARMUP_TOKENS`
  Local S2-Pro server startup warm-up에서 생성할 최대 token 수입니다. 기본값은 `32`입니다. vendored Fish Speech upstream의 기본 warm-up은 `1024` token이라 이 프로젝트에서는 startup 지연을 줄이기 위해 patch로 축소했습니다. 성능 디버깅 때만 값을 늘립니다.
...
### Vendored Fish Speech patch

이 프로젝트는 `vendor/fish-speech` 원본 중 S2-Pro 로컬 추론 성능과 startup 시간에 직접 영향을 주는 두 파일을 patch합니다.

- `vendor/fish-speech/fish_speech/models/text2semantic/inference.py`
  `decode_n_tokens()`에서 매 token마다 `SDPBackend.MATH`를 강제하던 wrapper를 제거했습니다. 이 wrapper가 있으면 PyTorch가 flash/mem-efficient attention을 선택하지 못해 RTX 5080 환경에서도 `0.1 tokens/sec` 수준으로 떨어질 수 있습니다.
- `vendor/fish-speech/tools/server/model_manager.py`
  server warm-up `max_new_tokens`를 고정 `1024`에서 `FISH_SPEECH_WARMUP_TOKENS` 기반 기본 `32`로 바꿨습니다. Warm-up은 readiness 확인 용도라 긴 생성을 할 필요가 없습니다.

업스트림과 다른 이 내용은 [Vendor upstream delta register](./25-vendor-upstream-deltas.md)에 재적용/제거 기준까지 기록합니다.
