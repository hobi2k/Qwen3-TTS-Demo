...
- `Applio 모델 블렌딩`
- `ACE-Step 작곡`
- `VibeVoice TTS / ASR / Model Tools`
- `CosyVoice TTS`
- `VoxCPM TTS`
- `Supertonic TTS`
- `Qwen 데이터셋 만들기`
- `Qwen 학습 실행`
- `VoiceBox 융합 / Clone / Clone + Instruct / Speaker Morph` *(hobi2k 고유 커스텀 개조 모델)*
- `S2-Pro 데이터셋 / 학습`
- `VibeVoice 데이터셋 / 학습`
- `Applio RVC 데이터셋 / 학습`
- `MMAudio 데이터셋 / 학습`
...
이 저장소의 목표는 `Qwen3-TTS`, `Fish Speech S2-Pro`, `VibeVoice`, `CosyVoice 3`, `VoxCPM2`, `Supertonic 3`, `Applio`, `MMAudio`, `ACE-Step`을 한데 묶고, 그 위에 **hobi2k 고유 커스텀 개조 모델 `VoiceBox`** 파이프라인을 별도로 제공해 아래 흐름을 실제로 시연할 수 있는 로컬 음성/음악 작업실을 만드는 것입니다.
...
- zero_shot / cross_lingual (한국어 권장) / instruct2 / sft / vc 5개 추론 모드
- vendor wrapper subprocess + `.venv-cosyvoice3` 격리 실행
- 학습은 llm/flow/hifigan 서브모듈 SFT (LoRA 미지원)
...
- ACE-Step, MMAudio, VibeVoice, Applio/RVC, CosyVoice 3, VoxCPM2는 subprocess 실행을 기준으로 하고, 실행 전 resident runtime을 비워 GPU 충돌을 줄임
- Supertonic 3는 onnxruntime in-process라 별도 unload 단계가 필요 없음 (FastAPI 프로세스 cache와 함께 정리됨)
...
- CosyVoice 3
  - `data/datasets/<dataset_id>/manifest.jsonl`에 `{"audio": ..., "text": ..., "speaker"?: ...}` 형식으로 정리
  - 결과는 CosyVoice 학습 탭에서 dataset_id로 참조하여 llm/flow/hifigan SFT에 사용
- VoxCPM2
  - 동일하게 `manifest.jsonl` (`audio`, `text`) 기반. CosyVoice와 같은 폴더 규약 공유
  - 결과는 VoxCPM2 LoRA 학습 탭에서 dataset_id로 참조
- Supertonic 3
  - ONNX 추론 전용이므로 학습 데이터셋이 필요 없음. 데이터셋 탭은 안내만 표시
- MMAudio
...
### 1. VoiceBox 설계 *(hobi2k 고유 커스텀 개조 모델)*

> **VoiceBox는 외부 vendor 모델이 아니라 이 저장소 소유자(hobi2k)의 고유 커스텀
> 개조 모델입니다.** upstream `Qwen3-TTS`에는 존재하지 않으며, FunAudioLLM/
> OpenBMB/Supertone 같은 외부 vendor 모델군과도 별개입니다. Qwen3-TTS의 `Base
> 1.7B` + `CustomVoice` 위에 hobi2k 고유 합성·재학습·morph 워크플로우를 얹어
> 만든 self-contained 모델군이며, 모든 학습/변환/추론 코드는 이 저장소의
> `qwen_extensions/`와 `scripts/`에 들어 있습니다. upstream 환원 대상이 아닙니다.

이 프로젝트 안에서는 `speaker_encoder`를 포함한 self-contained `CustomVoice` 계열을
hobi2k의 제품/모델 명칭으로 `VoiceBox`라고 부릅니다.
...
현재 `CustomVoice` 경로를 유지하면서도 학습 자립성을 갖춘 self-contained 확장형을 가리키는 hobi2k 고유 모델명입니다.
