...
- S2-Pro
  - `wav + .lab` 폴더 또는 proto 폴더를 지정
  - 결과는 Fish Speech `text2semantic_finetune`의 `lab_audio_dir` 또는 `proto_dir`로 연결
- VibeVoice
  - local dataset path, HF dataset id, train/validation JSONL, column 이름을 지정
  - 결과는 TTS Fine-tune 또는 ASR Fine-tune으로 연결
- Applio/RVC
  - 생성 갤러리 WAV 여러 개 또는 같은 화자의 WAV 폴더를 지정
  - 결과는 RVC preprocess/extract/train/index 파이프라인으로 연결
- ACE-Step
  - tensor 폴더, 오디오 폴더, dataset JSON 중 하나를 지정
  - 결과는 ACE-Step LoRA/LoKr trainer로 연결
- MMAudio
  - upstream `example_train` 검증 모드와 Hydra config 기반 실제 데이터 모드를 구분
  - 결과는 MMAudio full/continued training으로 연결
